...
(function () {
  ...
  function replay(entry) {
    ...
    if (entry.kind === "photo") {
      ...
      // MARK A REPLAY AS A REPLAY. A check-in made with no signal carries the
      // tap time in `at`, and the server honours that ONLY for a replay — a
      // live check-in is stamped by the server's own Cairo clock, so a phone
      // set to the wrong time cannot write the wrong hour onto a report.
      // Flagged just for the two stamped endpoints, so nothing else sees a
      // field it was not written to expect.
      const replayed = /\/(checkin|checkout)$/.test(entry.path || "");
      body = JSON.stringify(replayed ? { ...(entry.body || {}), __offline: 1 }
                                     : (entry.body || {}));
    }
  ...
  let flushing = false;
  // The API only ever answers JSON. An HTML body (or a WAF marker header) on a
  // 4xx means an edge/proxy blocked the request, not that the request was bad.
  function isEdgeChallenge(res) {
    ...
    try {
      ...
      for (const entry of items) {
        ...
        if (res.ok) {
          ...
          break;
        } else if (isEdgeChallenge(res)) {
          // Not our server answering: a CDN/WAF interstitial (Cloudflare's
          // "Just a moment…" is a 403 with an HTML body). The agent's write is
          // perfectly valid — keep it and retry once the browser holds a
          // clearance cookie, exactly like the 401 case.
          break;
